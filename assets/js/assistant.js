...
function getLocalAssistantReply(q){
    ...
    if(/score|evaluate|accuracy/.test(t))return 'AI scoring is an advanced feature with API key. In Basic mode, SpeakFlow uses local word-match feedback.';
...
function toggleAssistant(open){
    ...
    if(shouldOpen&&$('assist-msgs').children.length===0){
        pushAssistantMessage('bot',S.systemLanguage==='es'
            ?'Hola, soy tu asistente de SpeakFlow. Preguntame donde estan las funciones o como funcionan los modos.'
            :'Hi, I am your SpeakFlow assistant. Ask me where to find features or how modes work.');
    }
...
function sendAssistantMessage(){
    ...
    callGLM([
        {role:'system',content:'You are SpeakFlow Assistant. Help users navigate Home, Practice, Presentation, and Settings. Keep answers concise and practical.'},
        {role:'user',content:q}
